...
class NdcgMetric(ListBasedMetric):
    ...
    def evaluate_user(self, test_user_data: np.array,
        ...
        for j in range(min(rec_num, len(result_user_data))):
...
if __name__ == '__main__':
    ...
    #REVISION
    if (args['list_size'] is None):
        list_size = single_xpath(config.get_xml(), '/librec-auto/metric/list-size').text
        params = {'list_size': list_size}
    else:
        params = {'list_size': args['list_size']}  # ORIGINAL
    #REVISION
